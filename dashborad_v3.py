...
from diagram.heatmap_cve_technique import create_heatmap_cve_technique
from diagram.bubble_chart import create_bubble_chart
from diagram.heatmap_apt_cvss import create_heatmap_apt_cvss
from diagram.bar_chart_apt_cvss import create_bar_chart_apt_cvss
from diagram.histogram_cvss_apt import create_histogram_cvss  # Import the histogram function
from diagram.techniques_tactics_chart import create_techniques_tactics_chart
...
def render_content(cve_clicks, platform_clicks, cwe_clicks, selected_cves, selected_apts, selected_cwes):
    ...
    if cve_clicks > platform_clicks and cve_clicks > cwe_clicks:
        ...
        heatmap_cve_technique = create_heatmap_cve_technique(filtered_df)
        bubble_chart = create_bubble_chart(filtered_df)
        heatmap_apt_cvss = create_heatmap_apt_cvss(filtered_df)
        histogram_cvss = create_histogram_cvss(filtered_df)  # Generate the histogram
        bar_chart_apt_cvss = create_bar_chart_apt_cvss(df)
        content = html.Div([
            html.H3('CVE-Related Visualizations'),
            dcc.Graph(figure=bar_chart),
            dcc.Graph(figure=scatter_plot),
            dcc.Graph(figure=heatmap_fig),
            dcc.Graph(figure=heatmap_cve_technique),
            dcc.Graph(figure=bubble_chart),
            dcc.Graph(figure=heatmap_apt_cvss),
            dcc.Graph(figure=histogram_cvss),
            dcc.Graph(figure=bar_chart_apt_cvss)
        ])
        ...
        techniques_tactics_chart = create_techniques_tactics_chart(df)
        ...
        content = html.Div([
            ...
            dcc.Graph(figure=bubble_chart),
            dcc.Graph(figure=techniques_tactics_chart),
